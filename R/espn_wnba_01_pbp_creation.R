...
wnba_pbp_games <- function(y){
  ...
  if(nrow(pbp_g)>1){
    pbp_g <- pbp_g %>%
      wehoop:::make_wehoop_data("ESPN WNBA Play-by-Play Information from wehoop data repository",Sys.time())
    ...
    data.table::fwrite(pbp_g, file=paste0("wnba/pbp/csv/play_by_play_",y,".csv.gz"))
    
    ifelse(!dir.exists(file.path("wnba/pbp/qs")), dir.create(file.path("wnba/pbp/qs")), FALSE)
    qs::qsave(pbp_g,glue::glue("wnba/pbp/qs/play_by_play_{y}.qs"))
  ...
  rm(pbp_g)
  gc()
  cli::cli_process_done(msg_done = "Finished wnba play_by_play parse for {y}!")
  return(NULL)
}
